...
(function () {
  ...
  function heroCardHTML(g) {
    return (
      '<a class="hero__fcard" href="' + esc(g.link) + '" target="_blank" rel="noopener">' +
        '<div class="art' + (g.thumbnail ? "" : " art--fallback") + '" style="' + (g.thumbnail ? "background-image:url('" + esc(g.thumbnail) + "')" : "") + '"></div>' +
        '<div class="body">' +
          '<div class="ttl">' + esc(g.name) + "</div>" +
          '<div class="by">By ' + esc(g.creator || "Zap Works") + (g.creatorVerified ? " " + VERIFIED : "") + "</div>" +
        "</div>" +
      "</a>"
    );
  }
  ...
  function renderHero(games) {
    ...
    // three floating frames
    host.innerHTML =
      '<div class="hero__float">' +
        '<div class="hero__fcard hero__fcard--0">' + heroCardHTML(ordered[0] || games[0]) + "</div>" +
        '<div class="hero__fcard hero__fcard--1">' + heroCardHTML(ordered[1] || games[0]) + "</div>" +
        '<div class="hero__fcard hero__fcard--2">' + heroCardHTML(ordered[2] || games[0]) + "</div>" +
      "</div>";

    const cards = host.querySelectorAll(".hero__fcard");

    function fill(card, g) {
      if (!g) return;
      const a = card.querySelector(".art");
      a.className = "art" + (g.thumbnail ? "" : " art--fallback");
      a.style.backgroundImage = g.thumbnail ? "url('" + g.thumbnail + "')" : "none";
    ...
    // cycle through the games in groups
    let base = 0;
    function tick() {
      ...
      cards.forEach((card, i) => {
        ...
        setTimeout(() => {
          fill(card, ordered[(base + i) % ordered.length]);
          requestAnimationFrame(() => (card.style.opacity = "1"));
        }, 120);
      });
      base = (base + 1) % ordered.length;
    }

    function start() {
      stop();
      if (ordered.length > 3) heroTimer = setInterval(tick, 4500);
    }
    function stop() {
