...
(function () {
    ...
    return (
      '<a class="group-card" href="' + esc(g.link) + '" target="_blank" rel="noopener">' +
        ...
        '<span class="group-card__info">' +
          ...
          '<span class="group-card__members">' + fmt(g.members) + " members<span class=\"delta\" id=\"dlg-" + g.groupId + "\"></span></span>" +
        "</span>" +
  ...
  function renderStats(meta) {
    setNum("statMembers", "members", meta.totalMembers, "full");
    setNum("statPlaying", "playing", meta.totalPlaying, "full");
    setNum("statVisits", "visits", meta.totalVisits, "full");
  ...
  function arrowFor(key, value) {
    if (prev[key] == null || value == null) return { ch: "", cls: "delta" };
    if (value > prev[key]) return { ch: "▲", cls: "delta delta-up" };
    if (value < prev[key]) return { ch: "▼", cls: "delta delta-down" };
    return { ch: "", cls: "delta" };
  }
  ...
  function setNum(id, key, value, mode) {
    const el = document.getElementById(id);
    if (!el) return;
    const arrow = arrowFor(key, value);
    const txt = mode === "full" ? fmtFull(value) : fmt(value);
    const d = document.getElementById("dl-" + id);
    if (d && el.contains(d)) {
      // delta lives inside the value element -> rebuild so we don't wipe it
      el.innerHTML = esc(txt) + '<span class="' + arrow.cls + '" id="dl-' + id + '">' + arrow.ch + "</span>";
    } else {
      el.textContent = txt;
      if (d) {
        d.textContent = arrow.ch;
        d.className = arrow.cls;
      }
    }
    prev[key] = value;
  }
  ...
  function groupDeltas(groups) {
    groups.forEach((g) => {
      const d = document.getElementById("dlg-" + g.groupId);
      if (!d) return;
      if (prevGroups[g.groupId] != null && g.members != null) {
        if (g.members > prevGroups[g.groupId]) {
          d.textContent = "▲";
          d.className = "delta delta-up";
        } else if (g.members < prevGroups[g.groupId]) {
          d.textContent = "▼";
          d.className = "delta delta-down";
        } else {
          d.textContent = "▲";
          d.className = "delta delta-up";
        }
