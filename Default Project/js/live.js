...
(function () {
  ...
  // ---- hero: thumbnail collage + featured game card ----
  let heroTimer = null;

  function heroCardHTML(g, i) {
    return (
      '<a class="hero__card' + (i === 0 ? " is-active" : "") + '" href="' + esc(g.link) + '" target="_blank" rel="noopener">' +
        '<div class="hero__card__art' + (g.thumbnail ? "" : " hero__card__art--fallback") + '" style="' + (g.thumbnail ? "background-image:url('" + esc(g.thumbnail) + "')" : "") + '"></div>' +
        '<div class="hero__card__body">' +
          '<div class="hero__card__title">' + esc(g.name) + "</div>" +
          '<div class="hero__card__by">By ' + esc(g.creator || "Zap Works") + (g.creatorVerified ? " " + VERIFIED : "") + "</div>" +
        "</div>" +
      "</a>"
    );
  }
  ...
  function renderHero(games) {
    ...
    const ordered = games.slice().sort((a, b) => (b.playing || 0) - (a.playing || 0));

    if (backdrop) {
      const tiles = ordered.slice(0, 18);
      backdrop.innerHTML = tiles
        .map((g) =>
          '<div class="tile' + (g.thumbnail ? "" : " tile--fallback") + '" style="' + (g.thumbnail ? "background-image:url('" + esc(g.thumbnail) + "')" : "") + '"></div>'
        )
        .join("");
    }
    ...
    featured.innerHTML =
      '<div class="hero__stage">' + top.map((g, i) => heroCardHTML(g, i)).join("") + "</div>" +
      '<div class="hero__dots">' + top.map((g, i) => '<button class="hero__dot' + (i === 0 ? " is-active" : "") + '" data-i="' + i + '" aria-label="Game ' + (i + 1) + '"></button>').join("") + "</div>";

    const cards = featured.querySelectorAll(".hero__card");
    const dots = featured.querySelectorAll(".hero__dot");
    let idx = 0;
    ...
    function start() {
      stop();
      if (cards.length > 1) heroTimer = setInterval(() => show((idx + 1) % cards.length), 5000);
    }
    function stop() {
