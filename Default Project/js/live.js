...
(function () {
  ...
  function renderHero(games) {
    ...
    function fill(card, g) {
      ...
      by.innerHTML = "By " + esc(g.creator || "Zap Works") + (g.creatorVerified ? " " + VERIFIED : "");
    }

    // cycle through the games — a smooth simultaneous crossfade
    let base = 0;
    let lock = false;
    function tick() {
      if (lock || cards.length < 3) return;
      lock = true;
      base = (base + 1) % ordered.length;
      cards.forEach((card) => (card.style.opacity = "0"));
      setTimeout(() => {
        cards.forEach((card, i) => fill(card, ordered[(base + i) % ordered.length]));
