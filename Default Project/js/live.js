...
(function () {
  ...
  function gameCardHTML(g, i) {
    const thumb = g.thumbnail
      ? '<span class="gcard__thumb" style="background-image:url(' + "'" + esc(g.thumbnail) + "'" + ')"></span>'
      : '<span class="gcard__thumb gcard__thumb--fallback"></span>';
    return (
      '<a class="gcard" href="' + esc(g.link) + '" target="_blank" rel="noopener" data-players="' + (g.playing || 0) + '" data-visits="' + (g.visits || 0) + '">' +
        thumb +
        '<span class="gcard__name">' + esc(g.name) + "</span>" +
        '<span class="gcard__by">By ' + esc(g.creator || "Zap Works") + (g.creatorVerified ? " " + VERIFIED : "") + "</span>" +
        '<span class="gcard__stat"><span>Playing</span><b>' + fmt(g.playing) + "</b></span>" +
        '<span class="gcard__stat"><span>Plays</span><b>' + fmt(g.visits) + "</b></span>" +
      "</a>"
    );
  }
  ...
  function heroFill(card, g) {
    if (!g || !card) return;
    card.setAttribute("href", g.link || card.getAttribute("href"));
  ...
  function renderGraph(history) {
    ...
    // ---- dots are HTML (not SVG) so they stay perfectly round ----
    const last = pts[pts.length - 1];
    const endDot = document.createElement("div");
    endDot.className = "live-end-dot";
    endDot.style.left = (last[0] / W) * 100 + "%";
    endDot.style.top = (last[1] / H) * 100 + "%";
    el.appendChild(endDot);

    const hoverDotEl = document.createElement("div");
    hoverDotEl.className = "live-hover-dot";
    el.appendChild(hoverDotEl);
    ...
    svg.addEventListener("mousemove", (e) => {
      ...
      hoverDotEl.style.left = (p[0] / W) * 100 + "%";
      hoverDotEl.style.top = (p[1] / H) * 100 + "%";
      hoverDotEl.style.display = "block";
    ...
    // x-axis labels (oldest -> newest); show dates when the window spans days
    if (axis) {
      axis.innerHTML = "";
      const n = Math.min(6, points.length);
      const spanDays = (maxT - minT) > 20 * 3600 * 1000;
      for (let i = 0; i < n; i++) {
        ...
        axis.innerHTML += "<span>" + esc(label) + "</span>";
      }
    }
  }
  ...
  function apply(stats) {
    ...
    renderGrid(document.getElementById("featuredGrid"), sortGames(fullGames, "most").slice(0, 5), gameCardHTML);
    ...
    if (meta) renderHero(fullGames);
  }

  async function load() {
    // 1) render instantly from the bundled fallback (names only) — no "Loading..."
    apply(null);

    // 2) then hydrate with live data
