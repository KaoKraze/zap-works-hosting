...
function setupSorting() {
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const cards = Array.from(grid.querySelectorAll(".gcard")).sort((a, b) => {
...
// Contact form — opens a draft to zapworks@outlook.com (temporary)
function setupContactForm() {
  const form = document.getElementById("contactForm");
  if (!form) return;

  const CONTACT_EMAIL = "zapworks@outlook.com";
  ...
  form.addEventListener("submit", (e) => {
    ...
    const subject = (form.querySelector("#subject") && form.querySelector("#subject").value) || "General inquiry";
    ...
    status.textContent = "Opening your email app to send to " + CONTACT_EMAIL + "... Thanks" + (name ? ", " + name : "") + "!";
...
function setupReveal() {
  ...
  const els = document.querySelectorAll(".section, .partners-band, .cta");
...
// Smooth cross-page transition: fade out on internal navigation
function setupPageTransitions() {
  ...
  // restore state when using back/forward (bfcache)
